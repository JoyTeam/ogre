...
mkdir build
cd build
cmake -DOGRE_STATIC=TRUE -DOGRE_BUILD_PLUGIN_BSP=FALSE -DOGRE_BUILD_PLUGIN_DOT_SCENE=FALSE -DOGRE_BUILD_PLUGIN_PCZ=FALSE -DOGRE_BUILD_COMPONENT_PAGING=FALSE -DOGRE_BUILD_COMPONENT_MESHLODGENERATOR=FALSE -DOGRE_BUILD_COMPONENT_TERRAIN=FALSE -DOGRE_BUILD_COMPONENT_VOLUME=FALSE -DOGRE_BUILD_COMPONENT_PROPERTY=FALSE -DOGRE_BUILD_PLUGIN_CG=FALSE -DOGRE_BUILD_COMPONENT_OVERLAY=FALSE -DOGRE_BUILD_COMPONENT_OVERLAY_IMGUI=FALSE -DOGRE_BUILD_COMPONENT_BITES=FALSE -DOGRE_BUILD_COMPONENT_BULLET=FALSE -DOGRE_BUILD_RENDERSYSTEM_D3D9=FALSE -DOGRE_BUILD_RENDERSYSTEM_GL3PLUS=FALSE -DOGRE_BUILD_RENDERSYSTEM_GLES2=FALSE -DOGRE_BUILD_RENDERSYSTEM_GL=TRUE -DOGRE_BUILD_PLUGIN_ASSIMP=FALSE -DOGRE_BUILD_TOOLS=FALSE -DOGRE_BUILD_XSIEXPORTER=FALSE -DOGRE_BUILD_LIBS_AS_FRAMEWORKS=FALSE -DOGRE_BUILD_TESTS=FALSE -DOGRE_BUILD_PLUGIN_FREEIMAGE=TRUE -DOGRE_BUILD_PLUGIN_EXRCODEC=FALSE -DOGRE_BUILD_PLUGIN_STBI=FALSE -DOGRE_CONFIG_ENABLE_MESHLOD=FALSE -DOGRE_CONFIG_ENABLE_DDS=FALSE -DOGRE_CONFIG_ENABLE_PVRTC=FALSE -DOGRE_CONFIG_ENABLE_ETC=FALSE -DOGRE_CONFIG_ENABLE_ASTC=FALSE ..
```

Then copy generated include files to Platform/<platform>/include

```shell
cp include/* to ../Platform/<platform>/include
```

If any options have been changed, remove contents of the build directory and rerun cmake.
